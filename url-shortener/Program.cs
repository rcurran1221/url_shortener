...
internal class Program
{
    ...
    //short access count as a dictionary for now
    //ideally, i would have a kafka topic and generate an event like {"shortUrl": "url", "access_count": 1}
    // and offload the summation of how many times a short url was accessed into another consumer service
    // that then updates a relational database for analytical workflows like reporting
    // or use another redis key per shorturl and use one of the atomic increment functions 
    // i will leave access count when a short url is deleted so we have historical access values
    private static Dictionary<string, int> shortAccessCount = new Dictionary<string, int>();

    // these three related data structures are probably better living together in their own class 
    // with access functions that ensure they remain in sync
    // and to ensure statistics are updated as needed
    // given the production implementation will likely use an out of process data store, i will leave them as is for now
    ...
    {
		// run without args to run a test scenario
        if (args.Length == 0)
        {
			...
			RunProgram("DELETE", "https://roburl.com/01234567", ""); // delete the custom short
			RunProgram("GET", "https://roburl.com/01234567", ""); // get deleted long, shouldnt return anything
            return;
        }
    ...
    private static void GetLongUrlFromShort(string shortUrl)
    {
        ...
        {
            if (shortAccessCount.TryGetValue(shortUrl, out int count))
            {
                int newCount = count + 1;
                shortAccessCount[shortUrl] = newCount;
                Console.WriteLine($"short {shortUrl} has been accessed {newCount} times");
            }
            else
            {
                shortAccessCount[shortUrl] = 1;
                Console.WriteLine($"short {shortUrl} has been accessed {1} times");
            }
    ...
    // you could also think about using base 62 encoding, but it is complex and there is still a chance of collision
...
// retrospective:
// if i redid this challenge i would start with meaningful response types on the functions rather than relying 
// on console out to return response info to the user, this made things difficult to unit test and probably
// would be the first thing you would refactor when productionizing the code
// also because it is a cli app and not a long lived program, the lack of disk persistence makes it difficult to validate 
// behavior of multiple requests
